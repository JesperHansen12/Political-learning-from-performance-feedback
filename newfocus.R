...
# Lav om til df og lav ordbog med alle synonymer for elevers karakterer
dtm_df <- convert(dtm, to = "data.frame")

performance <- dictionary(list(performance = c("resultat*", "måling*", 
                                               "undervisnigseffekt*", "karakter*",
                                               "eksamen*", "afgangs*",
                                               "præst*", "gennemsnit*", "tests*",
                                               "nationale*", "prøver*")))
...
newest_dict$performance_sd <- newest_dict$performance /0.00240328
...
tidy_newdict$performance_sd <- tidy_newdict$performance /0.00105136
