...
# Lav om til df og lav ordbog med alle synonymer for forandring
...
new_dict <- dicto_dtm %>%
  mutate(kommune = str_extract_all(document, "^[A-z].*[^.//0-9//0-9-//0-9//0-9-//0-9//0-9//0-9//0-9//.pdf]"),
         ...
         total_words = forandr + unmatched,
         perc_change = forandr / total_words, 
...
new_dict$perc_change_sd <- new_dict$perc_change / 0.002478513

ggplot(new_dict, aes(x = dato, y = perc_change_sd, group = as.factor(skift_blok), fill = as.factor(skift_blok)))+
  geom_smooth(method = "loess")+
  geom_vline(aes(xintercept = as.Date("21-11-2017", format = "%d-%m-%Y")))+
  theme_minimal()+
  labs(x = "Year", y = "Change in documents", 
       title = "Figure 5. Change in documents over time (standardized)")+
  geom_text(x = as.Date("10-06-2018", format = "%d-%m-%Y"), y = 1.2, label = "Municipal election 21-11-2017")+
  scale_fill_discrete(name = "Change in \ngoverning coalition\nideology", 
                      breaks = c(1, 0),
                      labels = c("Yes", "No"))

# Formel test af om der var mere forandring efter valget
elect_test <- new_dict %>%
  mutate(election = ifelse(dato > as.Date("21-11-2017", format = "%d-%m-%Y"), yes = 1, no = 0))%>%
  filter(dato > as.Date("01-01-2017", format = "%d-%m-%Y") & dato < as.Date("01-06-2018", format = "%d-%m-%Y"))

mod <- lm(perc_change_sd ~ election + skift_blok + election*skift_blok, data = elect_test)
summary(mod)

mod <- lm(perc_change_sd ~ election + skift_blok + log(indbyggertal) + log(udg_pr_6_16) + reference + election*skift_blok, data = elect_test)
summary(mod)

# Samme test, bare med partiskift
ggplot(new_dict, aes(x = dato, y = perc_change_sd, group = as.factor(skift_parti), fill = as.factor(skift_parti)))+
  geom_smooth(method = "loess")+
  geom_vline(aes(xintercept = as.Date("21-11-2017", format = "%d-%m-%Y")))+
  theme_minimal()+
  labs(x = "Date", y = "Change in documents", 
       title = "Figure 5. Change in documents over time (standardized)")+
  geom_text(x = as.Date("10-06-2018", format = "%d-%m-%Y"), y = 1.2, label = "Municipal election 21-11-2017")+
  scale_fill_discrete(name = "Change in \ngoverning \nparty", 
                      breaks = c(1, 0),
...
mod <- lm(perc_change_sd ~ election + skift_parti + election*skift_parti, data = elect_test)
summary(mod)
...
social <- plm(perc_change_sd ~ social + log(exp) + log(indbyggertal), index = c("newyear", "kommune"), data = tidy_newdict,
              model = "pooling", effect = "twoways")
summary(hist)
stargazer(hist, social, type = "text")
...
stargazer(histlm, sociallm, type = "text")


# First differencing viser det samme
histfd <- plm(perc_change_sd ~ reference + log(exp) + log(indbyggertal), index = c("newyear", "kommune"), data = tidy_newdict,
              model = "fd", effect = "individual")
summary(histfd)

socialfd <- plm(perc_change_sd ~ social + log(exp) + log(indbyggertal), index = c("newyear", "kommune"), data = tidy_newdict,
                model = "fd", effect = "individual")
summary(socialfd)
...
ggplot(tidy_newdict, aes(perc_change_sd))+
  geom_density()+
  theme_bw()+
  labs(x = "% Change in documents (standardized)", y = "Density", title = "Figure D2. Density plot of change in documents")+
  xlim(0, 6)
